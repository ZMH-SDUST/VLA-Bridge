...
class DataFactory(Dataset):
    def __init__(self, name, partition, data_root, clip_model_name, zero_shot=False, zs_type='rare_first', num_classes=600, detr_backbone="R50"): ##ViT-B/16, ViT-L/14@336px
        ...
        elif self.clip_model_name == 'ViT-B/32':
            self.clip_input_resolution = 224
...
class CustomisedDLE(DistributedLearningEngine):
        ...
        for batch in tqdm(dataloader):
            ...
            for output, target in zip(outputs, batch[-1]):
                ...
                for hoi_idx in unique_hoi:
                    # ground truth中，哪些标签与该交互同类
                    gt_idx = torch.nonzero(target['hoi'] == hoi_idx).squeeze(1)
                    # prediction中，哪些标签与该交互同类
    ...
    @torch.no_grad()
    def test_hico_zzou(self, dataloader, args=None):
        net = self._state.net
        net.eval()
        dataset = dataloader.dataset.dataset
            ...
            for output, target in zip(outputs, batch[-1]):
                ...
                for hoi_idx in unique_hoi:
                    ...
                    if len(gt_idx):
                        ...
                        # print("HOI class ID is:", hoi_idx.item())
                        # print("Pred item indexes are: ", det_idx)
                        # print("match results is :", asr)
                        indices_of_1 = [index for index, value in enumerate(asr) if value == 1]
                        corresponding_det_idx = [det_idx[index] for index in indices_of_1]
                        if hoi_idx.item() not in result_dict.keys():
                            result_dict[hoi_idx.item()] = list()
                        result_dict[hoi_idx.item()].append(output['feats'][corresponding_det_idx, :])
            # if len(result_dict.keys()) > 50:
            #     print("early stop")
            #     break
        return result_dict
